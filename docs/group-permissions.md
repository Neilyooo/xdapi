...
核对时间：2026-05-27 00:26 CST
证据文件：`evidence/xdw_ratio_groups_20260518_1111.json`

## 总览结论

- 用户可见分组为 `1x`、`3x`、`5x`，分别显示为基础、加速、优先倍率组。
- 三个倍率组都能调用同一批 34 个公开 CMCC token 计费模型；分组差异体现在计费倍率和 group-level 请求额度，不再代表模型权限差异。
...
| --- | --- | ---: | ---: | ---: | --- | --- |
| `1x` | 基础 1x | 1.00x | 1.00x | 34 | `[60, 1000]` | 基础倍率组，普通用户默认迁移目标 |
| `3x` | 加速 3x | 3.00x | 1.00x | 34 | `[180, 1000]` | 更高倍率与更高 group-level 请求额度 |
| `5x` | 优先 5x | 5.00x | 1.00x | 34 | `[300, 1000]` | 最高倍率与最高 group-level 请求额度 |
...
| `/api/user/models` | 34 个模型 | 登录用户模型选择器可见完整模型目录 |
| `/api/pricing` | 34 个模型 | 匿名价格页可见完整模型目录 |
...
| `China Mobile MaaS - Moma` | `1x,3x,5x` | 7 | 已验证 Qwen 新模型，当前按临时 1x 展示 |
...
由于三个 CMCC 渠道都绑定 `1x,3x,5x`，下列 34 个模型对三个倍率组的可用性一致。
...
| 27 | `qwq-32b` | 1x / 3x / 5x |
| 28 | `qwen3.6-plus` | 1x / 3x / 5x |
| 29 | `qwen3-vl-plus` | 1x / 3x / 5x |
| 30 | `qwen-mt-plus` | 1x / 3x / 5x |
| 31 | `qwen3-omni-flash` | 1x / 3x / 5x |
| 32 | `qwen-mt-flash` | 1x / 3x / 5x |
| 33 | `qwen3.5-plus` | 1x / 3x / 5x |
| 34 | `qwen3-max` | 1x / 3x / 5x |

## 变更边界

- 本次没有改动上游中国移动 MaaS 资源；上游仍是只读供给层。
- 本次没有增加模型访问门槛；34 个公开模型仍对三个倍率组可用。
