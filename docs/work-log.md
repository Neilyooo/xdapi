...
## 2026-09-12 18:07 CST

### X5M DeepSeek V4.1 Flash 大项目缓存、容量与延迟验证未通过

- 使用不含客户数据的 `Atlas Ledger` 合成代码仓库进行大上下文测试；每个被上游接受的流式请求约为 `30,873` 输入 token。渠道仍为非公开 `测试` 分组的 `#89`，没有修改客户路由、容器、数据库、Redis、代理或网络。
- 32 次流式请求只成功 6 次：顺序 `4/8`、4 并发 `1/8`、8 并发 `1/16`。顺序成功请求总耗时 P50 `25.230s`、P95/最大 `80.985s`，首字 P50 `25.201s`、P95/最大 `80.981s`。
- XDAPI 分段日志显示 6 个成功请求全部使用同一条 HTTP/2 连接，其中 5 次为复用；请求写入上游 P95 只有 `0.996s`，但上游首响应 P95 达 `80.778s`，长等待不在 XDAPI 请求写入阶段。
- 绕过 XDAPI 的 X5M 最小直连请求返回 `429 rate_limit_error / gateway_queue_full`，内容为 pending requests 过多。随后以单并发、每 60 秒一次的方式再测 3 次，仍为 `3/3` 429，X5M 请求数、token 和扣费均无增加。
- 所有成功响应、XDAPI 日志和 X5M 账户统计中的缓存创建/读取 token 均为 0；首个成功请求后的 5 个成功候选为 `0/5` 命中。当前没有证据证明 X5M 为该模型提供可用的提示缓存。
- 另有一个大项目非流式请求返回 HTTP 200，但 XDAPI 未获得可解析的上游 usage：XDAPI 本地估算输入 `36,164`、输出 `0`，X5M 实际记录输入 `30,894`、输出 `373`。这会造成输出计量遗漏，是独立的协议/计费阻断项。
- 整个 X5M 验证会话累计 99 个被上游接受的请求、582,188 输入 token、12,264 输出 token，实际花费 `$0.0631244`；缓存创建和读取仍均为 0。
- 冷却后的最终审计中，XDAPI 状态、价格、充值、订阅计划和启用模型接口全部 HTTP 200；渠道 89 仍仅限非公开 `测试` 组，所有临时用户和令牌均已删除。
- 结论：该渠道当前只适合继续隔离验证，不具备公开运营条件。上线前至少需要 X5M 提升网关队列/并发容量并明确限额、实证缓存命中，以及修复非流式 usage 返回。
- 脱敏证据：[`x5m_deepseek_v41_flash_20260912.json`](../evidence/x5m_deepseek_v41_flash_20260912.json)

## 2026-09-12 17:35 CST

### 新增 X5M DeepSeek V4.1 Flash 隔离渠道并完成协议、缓存、延迟与并发验证

- 新增 live 渠道 `#89 X5M DeepSeek V4.1 Flash 可行性验证`，只映射 `deepseek-v4.1-flash-x5m -> deepseek-v4.1-flash`，仅绑定非公开 `测试` 分组，`auto_ban=0`；匿名 `/api/pricing` 不显示该别名，普通客户当前无法命中。
- 上游直连、管理员渠道测试、XDAPI `/v1/chat/completions`、以及上游/XDAPI `/v1/responses` 的流式和非流式组合均为 HTTP 200。干净 relay 批次为 `69/69` 成功；另有 Responses 四种组合 `4/4` 成功。
- 顺序 20 次请求延迟为 P50 `1.578s`、P95 `5.662s`、最大 `5.800s`。并发 8 的 24 次请求为 `24/24` 成功，P50 `1.900s`、P95 `3.509s`、最大 `5.116s`；本轮只证明低中并发可行性，不构成生产容量承诺。
- 五次相同长前缀请求在首请求后缓存命中 `0/4`；X5M 账户统计的 cache-read/cache-create token 也均为 0。当前不能把该线路宣传为支持提示缓存折扣。
- 整个验证会话由 X5M 记录 92 次请求、366,048 输入 token、11,793 输出 token，实际花费 `$0.041322`。观察到的单价为输入 `$0.10/M`、输出 `$0.40/M`，恰为当前 DeepSeek 闲时无缓存官方价格的三分之二。
- 当前 `tiered_expr` 仅用 X5M 美元成本数值作为内部测试计价门；XDAPI 前台显示人民币，因此这不是已批准的客户售价。公开前仍需确定人民币换算、利润、正式 alias 和目标业务分组。
- 新渠道写入数据库后，内存 distributor 约 35 秒才完成收敛；快速删建期间有 5 条成功测试日志仍携带已删除的探针渠道 ID，随后 64 条均稳定命中最终 `#89`。后续新增渠道必须等待能力缓存收敛后再判定路由结果。
- 临时测试用户和令牌均已删除；XDAPI 状态、定价、充值信息、订阅计划和启用模型接口均返回 HTTP 200。未修改公开分组、客户路由、容器、镜像、数据库、Redis、反向代理或网络配置。
- 脱敏证据：[`x5m_deepseek_v41_flash_20260912.json`](../evidence/x5m_deepseek_v41_flash_20260912.json)

## 2026-08-17 12:17 CST

### XDAPI-NG 完成多实例运行时、协议矩阵、恢复演练与安全加固阶段验收

- 在隔离环境完成 Redis 运行时租约、队列和熔断状态，并由两个 Platform API 实例共同提供服务；异步任务可以跨实例读取和推进，节点状态能够收敛。
- 标准验证矩阵覆盖 OpenAI Chat Completions、OpenAI Responses、Anthropic Messages、图像生成和异步视频五类协议，共 22 个验证尝试；请求、实际渠道、规范化 usage 与五维计费记录能够关联核对。
- 新增 Prometheus 指标和仅记录元数据的 JSONL 运行日志。客户、健康探针和故障注入流量继续分开统计；日志不保存 Prompt、Completion 或密钥字段，默认保留 30 天并自动清理过期运行日志。
- 容量与故障验证通过：基线 32/32、30 秒短时 soak 111/111，并覆盖排队、超时、熔断、节点摘除和租约恢复。该结果是阶段门禁，不等价于生产容量承诺。
- 完成自动备份和独立命名空间恢复演练，验证数据库导入、Redis、应用启动、镜像归档、校验和、无主机端口暴露以及受保护服务不变；演练资源按审计要求保留。
- 安全层完成 KMS-ready v2 信封、密钥不可回读与重包裹、配置编辑/验证/审批/审计四角色职责分离、SBOM 和依赖审计。`govulncheck` 因工具不可用被明确记录为待补门禁，其余本轮供应链检查通过。
- 最终聚合验收和证据清单校验通过；渠道 40 保持禁用，未接触生产 XDAPI、真实上游、支付或公网入口。
- 当前估算：整体平台工程完成度约 `80%`，隔离核心链路约 `90%`，生产就绪度约 `58%`。下一阶段进入不切流的生产就绪基础建设：高可用基础设施、外部可观测性与长稳压测、云 KMS、异地恢复、签名供应链和真实渠道灰度工具。

## 2026-08-16 23:35 CST

### XDAPI-NG 完成配置发布、三层验证与流量分类阶段验收

- 在隔离节点完成模型、物理资源池、逻辑渠道、模型映射、用户组、优先级、权重、并发、排队、重试、超时和熔断配置；同一个公共模型可以由多个渠道提供服务，客户无需更换模型名。
- 所有价格档位都独立保存输入、输出、缓存读取、5m 缓存创建和 1h 缓存创建。Claude 强制使用 Anthropic 协议并保留完整五维价格；Claude 与 OpenAI 都不继承模型家族默认值或历史 `1h = 1.6x`。
- 发布只能经过 `DRAFT -> VALIDATING -> APPROVED -> ENABLED`，包含乐观锁、脱敏差异、审计、主机侧备份、受保护容器检查和回滚引用；禁止直接修改生效配置。
- 标准验证覆盖上游直连、渠道和 Relay 三层，分别验证 OpenAI 与 Anthropic。Claude 固定连续请求五次，核对缓存创建/读取、实际渠道、请求 ID、Token、计费以及排队、重试、超时和熔断边界。
- 客户流量、健康探针和故障注入已分开存储和统计。迁移仅对 7 条有精确 fixture 标识的历史记录重新分类，并写入 7 条追加式审计；没有删除客户历史。模型广场现在只用客户流量判断健康，两个验收模型均为 `AVAILABLE / 100% / 10 个客户样本`。
- 最终验收：run `config-release-20260816T152336Z`、release `rel_9mqFt5Ukkd7HYBjTTPJ0Zw`、validation `val_gddmz4FCTh_nTHpUeu6htA`，13 个验证尝试全部通过，证据清单校验通过，临时验证 Token 已撤销。
- 隔离阶段目标已经完成；整体 XDAPI-NG 工程完成度估计约 `68%`，生产就绪度约 `50%`。下一阶段优先完成多实例运行时、完整文本/生图/视频协议、独立日志与指标、容量与故障测试、恢复演练和安全加固。
- 本轮只重建隔离节点的 `platform-api`；Edge、Relay、fake upstream、MySQL、Redis 与 fixture Relay 的容器身份和启动时间不变。渠道 40 仍为 `DISABLED`，生产 XDAPI、真实上游、公网入口与支付渠道均未接触或修改。

## 2026-08-15 18:55 CST

### XDAPI-NG 完成无需公网备案的隔离 TLS 与渠道 40 流程验收

- 在独立天翼云验证节点新增仅绑定 `127.0.0.1:19443` 的私有 TLS 网关，通过 SSH 本地转发验证准确域名、私有 CA、HTTP/1.1、HTTP/2、安全响应头和管理路径隔离；未开放公网 `80/443/8080`。
- 完成渠道 40 的真实上游标准化流程：`openai-gpt-5.6-sol-group -> gpt-5.6-sol`，包含直连非流式/流式、Relay 连续 5 次非流式、Relay 流式、未授权边界、请求关联、计费和回滚。
- Relay 五次稳定性请求总耗时为 min `1.764307s`、P50 `1.907115s`、P95/最大 `2.753928s`；首个请求后的四次均观察到缓存读取，命中率 `4/4`，累计缓存读取 `7,168` tokens。
- 5 分钟缓存创建价格保持独立可编辑但本轮上游 usage 未暴露创建 TTL，因此标记为“已配置、运行时未实证”；1 小时缓存创建继续保持“待审批”，没有继承 XDAPI 默认 `1.6x`。
- 验证完成后只停止隔离 TLS 网关和 SSH 转发，模型恢复 `DISABLED`，隔离节点只保留回环诊断入口；原生产域名与服务未修改、未重启，固定校验仍返回 HTTP 200。
- 该结果只证明功能管线、TLS 协议、稳定性、缓存、计费与回滚闭环。证据明确标记 `ISOLATED_TLS_ONLY` 和 `satisfies_public_ingress=false`；天翼备案接入、公网 TCP 443、公有 CA 和外网 HTTPS 仍需单独验收。
...
- 模型元数据已归入 live `OpenAI` 供应商（`vendor_id=8`），因此可在模型广场的 OpenAI 供应商筛选下显示。
...
- 由于商业上游价格尚未确认，当前使用显式、可编辑的 `price_pending` 表达式，不继承系统模型倍率。部署脚本最初将渠道禁用；供应商归类时 live 回读已为启用状态，因此正式给客户计费前必须替换占位价格。
