...
## 2026-05-27 00:26 CST 现网部署更新

- `China Mobile MaaS - Moma` 已作为独立 relay 渠道接入 XDAPI 公共配置，7 个已验证 Qwen 新模型正式进入公开目录。
- 当前公开模型数已回到 `34`，`/api/pricing` 与首页部署数同步更新。
- `gui-plus` 与 `glm-5.1` 仍未通过上游矩阵，保持不接入公开前台。
- 7 个 Moma relay 新模型当前按临时 `1x` 展示，后续如果补齐更细的官方价格原文，再单独修订价格字段。
...
| --- | --- |
| 公开模型数 | 34 |
| 新增候选模型公开状态 | 7 个已公开，2 个未通过 |
| 价格原文覆盖 | 26/34 个公开模型在已采集官方 token 价格文章中有对应价格行 |
| 价格原文缺口 | `deepseek-v4-flash` 与 7 个 Moma relay 新模型暂按临时 1x 展示，未在本轮采集到更细的官方价格表行 |
| 上下文/最大输出原文覆盖 | 7/34 个公开模型有官方 API 文档表格证据 |
| 上下文/最大输出原文缺口 | 多数纯文本聊天模型及 Moma relay 新模型在本轮采集到的官方帮助中心 API 文档中没有逐模型上下文/最大输出表格；这不是断言上游没有数据，只表示本轮没有采到可逐模型核对的原文证据 |
