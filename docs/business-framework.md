...
- Redis 统一承载节点租约、排队和熔断运行态；两个 Platform API 实例经隔离网关共同服务，异步任务能够跨实例读取和推进，不再依赖单进程内存状态。
- 协议矩阵已覆盖 OpenAI Chat Completions、OpenAI Responses、Anthropic Messages、图像生成和异步视频，并统一关联请求 ID、实际渠道、规范化 usage 和五维计费。
- 运行日志采用仅元数据 JSONL，默认保留 30 天并自动清理；Prompt、Completion 和密钥不进入运行日志。Prometheus 指标覆盖 TTFT、总延迟、吞吐、错误率、排队、重试、熔断和节点状态。
- 备份、独立恢复演练、KMS-ready v2 密钥信封、四角色职责分离、SBOM 与依赖审计已通过阶段验收。外部云 KMS、长时间 soak、异地恢复和签名制品仍是生产启用前置条件。
- 2026-08-17 聚合验收通过。渠道 40 保持禁用，生产 XDAPI、真实上游、公网入口和支付均未修改。
- 当前估算：整体平台工程完成度约 80%，隔离核心链路约 90%，生产就绪度约 58%。下一阶段只建设不切流的生产就绪基础，不直接迁移客户流量。
...
- 2026-09-12 18:07 CST `#89 X5M DeepSeek V4.1 Flash 可行性验证` 仍只绑定非公开 `测试` 分组。小请求 Chat Completions `69/69` 和 Responses `4/4` 通过，但约 30.9K 输入 token 的大项目流式压力仅 `6/32` 成功，4/8 并发阶段大量返回 X5M `gateway_queue_full`；成功候选缓存命中 `0/5`。该渠道不具备公开运营条件。
...
| `X5M DeepSeek V4.1 Flash 可行性验证` | `deepseek-v4.1-flash-x5m -> deepseek-v4.1-flash` | `测试`（非公开） | 小请求双协议通过；大项目容量 `6/32`、缓存 `0/5`，存在 queue-full 和非流式 usage 风险，禁止公开 |
